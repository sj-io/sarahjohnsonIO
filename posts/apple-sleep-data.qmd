...
```{r, echo=FALSE}
most_sleep <- slice_max(sleep_by_wday, wday_sleep)

least_sleep <- slice_min(sleep_by_wday, wday_sleep)
```

I get the most sleep on **`r most_sleep$wday`s** where I average **`r as.character(round(most_sleep$readable))`**. I sleep the least on **`r least_sleep$wday`s** with an average of **`r as.character(round(least_sleep$readable))`**. 
...
cycle_amounts <- cycle_by_day |> 
  filter(value != "Unspecified") |> 
  summarise(cycle_amount = sum(cycle_amount), 
            .by = "value") |> 
  mutate(pct = (as.numeric(cycle_amount) / 
                  sum(as.numeric(cycle_amount))) |> lp()
         )
```

Excluding unspecified sleep stages, core sleep makes up `r cycle_amounts[cycle_amounts$value == "Core", ]$pct` of my sleep cycles; deep sleep is `r cycle_amounts[cycle_amounts$value == "Deep", ]$pct`; and REM sleep makes up `r cycle_amounts[cycle_amounts$value == "REM", ]$pct`.
...
<center>
![](img/apple-health-sleep-graph.jpg){width=40%}
</center><br>
...
```{r echo=FALSE}
monthly_sleep <- cycle_activity |> 
  mutate(cycle_time = endDate - startDate) |> 
  summarise(daily_sleep = sum(cycle_time), .by = creationDate) |> 
  mutate(month = month(creationDate, label = TRUE, abbr = FALSE)) |> 
  left_join(sleep_start |> select(creationDate, start_time), by = "creationDate") |> 
  left_join(sleep_end |> select(creationDate, end_time), by = "creationDate") |> 
  summarise(avg_start = round(mean(start_time)),
            avg_end = round(mean(end_time)),
            avg_daily = mean(daily_sleep),
            .by = "month") |> 
  mutate(across(c(avg_start, avg_end), ~ hms::as_hms(.x - 86400)),
         avg_daily = seconds_to_period(avg_daily))

month_early_bed <- slice_min(monthly_sleep, avg_start)
month_late_bed <- slice_max(monthly_sleep, avg_start)
month_late_wake <- slice_max(monthly_sleep, avg_end)
month_early_wake <- slice_min(monthly_sleep, avg_end)
month_least <- slice_min(monthly_sleep, avg_daily)
month_max <- slice_max(monthly_sleep, avg_daily)
```

The month where I averaged the most sleep was **`r month_max$month`** with an average of **`r as.character(round(month_max$avg_daily))`** per night. The month with the least was **`r month_least$month`** with an average of **`r as.character(round(month_least$avg_daily))`** per night. I woke up the earliest on average in **`r month_early_wake$month`** at around **`r as.character(month_early_wake$avg_end)`** and the latest in **`r month_late_wake$month`** at around **`r as.character(month_late_wake$avg_end)`**. I went to bed the earliest on average in **`r month_early_bed$month`** at around **`r as.character(month_early_bed$avg_start)`** and the latest in **`r month_late_bed$month`** at around **`r as.character(month_late_bed$avg_start)`**.
...
```{r, fig.width=12, fig.height=4.29, echo=FALSE}
#| column: screen-inset-right
sleep_heatmap |> 
...
all_nighters <- tibble(
  creationDate = c(
    "2023-08-03", "2023-08-11", "2023-10-22", "2023-11-18",
    "2024-01-09", "2024-03-14", "2024-03-17", "2024-03-20"
    ),
  daily_sleep = c(
    "0:00:00", "0:00:00", "3:00:00", "7:30:00",
    "1:40:00", "0:00:00", "0:00:00", "3:00:00"
    )
) |> 
  mutate(creationDate = date(creationDate),
         daily_sleep = hms::as_hms(daily_sleep),
         wday = wday(creationDate, abbr = TRUE, label = TRUE),
         year = year(creationDate),
         week = as.numeric(week(creationDate)),
         week = ifelse(year == as.numeric(2024), week + 52, week)
         )

sleep_debt <- sleep_heatmap |> 
  bind_rows(all_nighters) |> 
  # mutate(sleepy_debt = 28800 - as.numeric(daily_sleep))  # 8 hours
  mutate(sleepy_debt = 25200 - as.numeric(daily_sleep)) # 7 hours
weekly_sleep_debt <- sleep_debt |> 
  summarise(weekly_debt = sum(sleepy_debt), .by = week)
weekday_sleep_debt <- sleep_debt |> 
  filter(!str_starts(wday, "S")) |> 
  summarise(weekly_debt = sum(sleepy_debt), .by = week)

ggplot() +
  # geom_col_rounded(data = weekly_sleep_debt, aes(x = week, y = weekly_debt)) +
  geom_col_rounded(data = sleep_debt, aes(x = creationDate, y = sleepy_debt))
```


```{r, echo=FALSE}
ggplot(data = weekday_sleep_debt, aes(x = week, y = weekly_debt, label = paste(week, "|", round(weekly_debt/3600, 2)))) +
  geom_col_rounded(fill = "grey90") +
  geom_text(size = 3, color = "red", check_overlap = TRUE, fontface = "bold", angle = 90, hjust = "inward") +
  theme_void()
```


```{r, echo=FALSE}
ggplot(data = weekly_sleep_debt, aes(x = week, y = weekly_debt, label = paste(week, "|", round(weekly_debt/3600, 2)))) +
  geom_col_rounded(fill = "grey90") +
  geom_text(size = 3, color = "red", check_overlap = TRUE, fontface = "bold", angle = 90, hjust = "inward") +
  theme_void()
  # geom_col_rounded(data = sleep_debt, aes(x = creationDate, y = sleepy_debt))
...
Since I began tracking my sleep, my total sleep debt is **`r round(sum(weekly_sleep_debt$weekly_debt)/3600, 2)` hours**.
