...
cbsa <- core_based_statistical_areas() %>% filter(str_detect(NAME, "Memphis")) 
csaCounties <- map_dfr((memST), ~{counties(.x)}) %>% st_filter(csa, .predicate = st_within)
counties <- csaCounties[cbsa, op = st_within]
sld <- state_legislative_districts("TN") %>% st_filter(cbsa, .predicate = st_within)
places <- map_dfr((memST), ~{places(.x)}) %>% st_filter(cbsa, .predicate = st_within)
...
school <- school_districts("TN") %>% st_filter(shelby, .predicate = st_within)
cong <- congressional_districts("TN") %>% st_filter(shelby, .predicate = st_within)
```
...
Below is an interactive map of boundaries related to the Memphis area. Layers will stack in the order they are selected.
...
This post, the map, and all data collection was completed using [R](https://www.r-project.org/)/[RStudio](https://www.rstudio.com/products/rstudio/). Boundaries were pulled using the **tigris** package. A full list of available datasets and their corresponding functions is available in Chapter 5 of the book [Analyzing US Census Data](https://walker-data.com/census-r/census-geographic-data-and-applications-in-r.html#data-availability-in-tigris).
