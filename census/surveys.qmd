...
```{r}
ACS1 <- read_csv("data/table/census/acs/acs1/B25-hsg.csv") %>%
  mutate(dataset = "ACS1") %>%
  filter(variable == "B25001_001" & GEOID == 47157)
ACS5 <- read_csv("data/table/census/acs/acs5/B25-hsg.csv") %>%
  mutate(dataset = "ACS5") %>%
  filter(variable == "B25001_001")
PEP <- read_csv("data/table/census/pep-2010-2019-HUEST.csv") %>%
  mutate(dataset = "PEP") %>%
