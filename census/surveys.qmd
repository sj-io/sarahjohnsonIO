...
The Census conducts over 130 surveys and programs each year, but a handful are especially useful for housing research. In this post I will compare four datasets that provide information on population and housing units: the American Community Survey (1-year & 5-year) and the Population Estimates Program.
...
# Doesn't work yet. 
# Five year data will release on 12/8/22
B25 <- get_acs(
  survey = "acs5",
  geography = "county",
  state = "TN", 
  ...
  mutate(year = 2021, .before = GEOID)

ACS5 <- rbind(ACS5, B25)

write_csv(ACS5, "data/ACS5.csv")

# PEP releases eventually
# HU <- get_estimates(
#   geography = "county",
#   state = "TN",
#   county = "Shelby",
...
gg <- ggplot(hsg, aes(x = year, y = estimate, color = dataset)) +
  ...
  labs(title = "Housing Units in Shelby County, 2010-2021",
       y = "",
       x = "",
       color = "Dataset:") +
  theme_minimal() +
