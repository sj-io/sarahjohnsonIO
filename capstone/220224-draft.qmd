...
```{r HHRS}
HHRS <- read_csv("data/hud/HHRS-hazards.csv")
...
v_cost_burden <- v_acs5 %>% 
  ...
  mutate(tenure = str_extract(label, "Owner|Renter"), 
         ...
         across(c(income, burden), ~ str_replace_all(.x, v_cb_reformat)),
         burden = na_if(burden, ""),
         cb_sum = case_when(
           str_detect(burden, "20|No cash rent") ~ "Affordable",
           str_detect(burden, "30|negative income") ~ "Burden"
         ),
         .after = name) %>% 
...
graphi_inc_pl <- graphi %>% 
  filter(!is.na(income) & !is.na(cb_sum)) %>% 
  group_by(GEOID, NAME, tenure, income, cb_sum) %>% 
  summarise(estimate = sum(estimate), inc_sum = inc_sum) %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate(pct = estimate / inc_sum,
         NAME = str_remove_all(NAME, " (city|town).*$"),
         NAME = fct_inorder(NAME),
...
graphi_inc_pl %>% 
  ...
  theme_minimal() +
  labs(title = "Cost Burden by Tenure by Income",
       subtitle = "Memphis, TN",
       caption = "2017-2021 5-Year ACS Estimates",
       y = "",
       x = "",
...
```{r vGRAPHIdet}
v_renter_cb <- v_acs5 %>% 
  filter(str_starts(name, "B25070")) %>% 
  mutate(burden = str_remove(label, ".*:(!!)?"),
         burden = str_replace_all(burden, v_cb_reformat),
         burden = na_if(burden, ""),
         cb_sum = case_when(
           str_detect(burden, "[12][05]") ~ "Affordable",
           str_detect(burden, "[34][05]") ~ "Burden",
           str_detect(burden, "50") ~ "Extreme burden",
           TRUE ~ burden
           ),
         .after = name) %>%
  select(-c(label:geography))
...
renter_cb <- hsg %>% 
  ...
  left_join(v_renter_cb, by = c("variable" = "name"))

renter_cb_sum <- renter_cb %>% 
  filter(!is.na(cb_sum)) %>% 
  group_by(GEOID, NAME, cb_sum) %>% 
  summarise(estimate = sum(estimate), summary_est = summary_est) %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate(pct = estimate / summary_est,
         NAME = str_remove_all(NAME, " (city|town).*$"),
         NAME = fct_reorder(NAME, summary_est, max, .desc = TRUE))
```

```{r ggGRAPHIdetMem, fig.cap="Cost burden for renters"}
renter_cb_sum %>% 
  ggplot(aes(x = NAME, y = pct, fill = cb_sum)) +
  geom_col(position = position_stack(reverse = TRUE)) +
  scale_y_continuous(labels = scales::label_percent()) +
  scale_fill_manual(values = c("mediumseagreen", "goldenrod", "tomato", "gray")) +
  theme_minimal() +
  labs(title = "Housing Costs for Renters",
       subtitle = "Places in Shelby County, TN",
       caption = "2017-2021 ACS 5-Year Estimates",
       x = "", y = "", fill = "Housing Costs")
```
...
```{r vGRAPHIdet2}
v_renter_cb_detail <- v_acs5 %>% 
  filter(str_starts(name, "B25074")) %>% 
  mutate(income = str_extract(label, "(?<=Total:!!).*(?=:)"), 
         burden = str_remove(label, ".*:(!!)?"),
         across(c(income, burden), ~ str_replace_all(.x, v_cb_reformat)),
         burden = na_if(burden, ""),
         cb_sum = case_when(
...
renter_cb_detail <- hsg %>% 
  filter(str_starts(variable, "B25074")) %>% 
  left_join(v_renter_cb_detail, by = c("variable" = "name")) %>% 
  group_by(GEOID, income) %>% 
  mutate(inc_sum = max(estimate)) %>% 
  ungroup()

renter_cb_detail_sum <- renter_cb_detail %>% 
  filter(!is.na(burden)) %>% 
  mutate(income = fct_inorder(income)) %>% 
  group_by(GEOID, NAME, income, cb_sum) %>% 
  summarise(estimate = sum(estimate), inc_sum = inc_sum) %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate(pct = estimate / inc_sum,
         NAME = str_remove_all(NAME, " (city|town).*$"),
         NAME = fct_reorder(NAME, inc_sum, max, .desc = TRUE),
         income = fct_rev(income))
```
...
renter_cb_detail_sum %>% 
  ggplot(aes(x = income, y = estimate, fill = cb_sum)) + 
  geom_col(position = "stack") +
  coord_flip() +
  scale_fill_manual(values = c("mediumseagreen", "goldenrod", "tomato", "gray")) +
  scale_y_continuous(labels = scales::label_comma(scale_cut = cut_short_scale())) +
...
```{r}
renter_cb_detail_sum %>% 
  ggplot(aes(x = income, y = pct, fill = cb_sum)) + 
  geom_col(position = "stack") +
  coord_flip() +
  scale_fill_manual(values = c("mediumseagreen", "goldenrod", "tomato", "gray")) +
  scale_y_continuous(labels = scales::label_percent()) +
...
```{r ggUnits, fig.cap="Units in structure", eval=FALSE}
...
```{r fig.cap="Year structure built", eval=FALSE}
