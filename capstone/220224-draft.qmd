...
v_acs5 <- read_csv("data/census/var/acs5-2021.csv")
```
...
rent_cbsa <- hsg %>%
  filter(variable == "B25003_003" &
           str_detect(NAME, "Metro Area") &
           year == 2021) %>%
  mutate(pct = estimate / summary_est,
         NAME = str_remove_all(NAME, "(-|,).*$")) %>% 
  select(NAME, summary_est, estimate, pct) %>% 
  arrange(desc(pct))
```
...
datatable(
  ...
  colnames = c('Metro Area', 'Total Households', 'Renters', '%'),
  rownames = FALSE,
  style = "bootstrap"
  ) %>%
  formatPercentage(4, mark = ".", digits = 1) %>%
...
datatable(
  rent_place,
  rownames = FALSE,
  colnames = c('Place', 'Total Households', 'Renters', '%'),
...
library(sf)
tract <- read_sf("data/geo/tract/tracts_21.shp") 

rent_tract <- tract %>% 
  left_join(rent_tract, by = "GEOID")
```

```{r map}
library(mapview)
mapview(rent_tract, zcol = "pct", 
        label = paste0(rent_tract$NAME, ": ", 
                       round(rent_tract$pct * 100, 1), "% (", 
                       rent_tract$estimate, " renters)"))
```
...
rent_race <- hsg %>% 
  filter(str_detect(NAME, "Memphis") &
           str_starts(variable, "B25003[:alpha:]_003")) %>% 
  mutate(estimate_mem = rent_mem$estimate,
         summary_mem = rent_mem$summary_est,
         variable = recode(variable, !!!v_race),
         rent_pct = estimate / estimate_mem,
         hh_pct = summary_est / summary_mem,
         diff = round((rent_pct - hh_pct) * 100, 1)) %>% 
  arrange(desc(diff)) %>% 
  select(variable, rent_pct:diff)
...
datatable(
  rent_race,
  rownames = FALSE, 
  colnames = c('Race/Ethnicity', 'Renters', 'Households', 'Difference'),
  caption = "Table 3: Race/Ethnicity of Renters in Memphis",
  style = "bootstrap",
  options = list(pageLength = 4),
  autoHideNavigation = getOption("DT.autoHideNavigation", TRUE)
  ) %>% 
  formatPercentage(2:3, mark = ".", digits = 1) %>% 
  formatRound(4)
```
...
race_tract <- hsg %>% 
  filter(str_starts(variable, "B25003[:alpha:]_003") &
           str_length(GEOID) == 11) %>%
  mutate(pct = (estimate / summary_est) * 100,
         GEOID = as.character(GEOID),
         variable = recode_factor(variable, !!!v_race))

race_tract_geo <- tract %>% 
  left_join(race_tract, by = "GEOID") %>% 
...
tm_shape(tract) +
  tm_polygons(col = "white",
              border.col = "grey") +
  tm_shape(race_tract_geo) +
  tm_dots(col = "variable",
          palette = "Set1",
          size = 0.07,
          title = "1 dot = 50 households") +
  tm_layout(legend.outside = TRUE,
            title = "Race/ethnicity of renters,\n2017-2021 US Census")
```
...
```{r vGRAPHIown}
cost_burden <- hsg %>% 
  filter(str_detect(variable, "B25106") &
           str_detect(NAME, "Memphis"))

v_cost_burden <- v_acs5 %>% 
  filter(str_detect(name, "B25106_")) %>% 
  mutate(tenure = str_extract(label, "Owner|Renter"), 
         income = str_extract(label, "(?<=units:!!).*(?=:)"), 
         burden = str_remove(label, ".*:(!!)?"),
         across(c(income, burden), ~ str_replace(.x, "Less than ", "<")),
         across(c(income, burden), ~ str_replace(.x, " to ", "-")),
         across(c(income, burden), ~ str_replace(.x, " or more", "+")),
         burden = str_replace(burden, " percent", "%"),
         .after = name) 
...
AHSimport <- read_csv("data/census/ahs-2019-hsg-quality.csv")
```
