...
geoid <- read_csv("data/census/var/geoid.csv")
```
...
```{r vPlaces}
v_places <- c("Memphis", "Lakeland", "Bartlett", 
...
library(sf)
tract <- read_sf("data/geo/tract/tract-21.shp") 
...
mapview(rent_tract, zcol = "pct", 
        label = paste0(rent_tract$NAMELSAD, ": ", 
...
v_cost_burden <- v_acs5 %>% 
  ...
  mutate(tenure = str_extract(label, "Owner|Renter"), 
         ...
         burden = na_if(burden, ""),
         cb_sum = case_when(
           str_detect(burden, "20|No cash rent") ~ "Affordable",
           str_detect(burden, "30|negative income") ~ "Burdened"
         ),
         .after = name) %>% 
  select(-c(label:geography))

graphi <- hsg %>% 
  filter(str_starts(variable, "B25106")) %>% 
  left_join(v_cost_burden, by = c("variable" = "name")) %>% 
  group_by(GEOID, tenure) %>% 
  mutate(ten_sum = max(estimate)) %>% 
  ungroup()
```

```{r GRAPHItotal}
graphi_pl_sum <- graphi %>% 
  filter(!is.na(cb_sum)) %>% 
  group_by(GEOID, NAME, tenure, cb_sum) %>% 
  summarise(estimate = sum(estimate), ten_sum = ten_sum) %>% 
  arrange(desc(ten_sum)) %>% 
  ungroup() %>% 
  distinct() %>% 
  mutate(pct = estimate / ten_sum,
         NAME = str_remove_all(NAME, " (city|town).*$"),
...
```{r}
graphi_pl_sum %>% 
  ggplot(aes(x = tenure, y = estimate, fill = cb_sum)) +
  ...
  labs(title = "Cost Burden by Tenure",
       subtitle = "Places in Shelby County, TN",
       caption = "2017-2021 5-Year ACS Estimates",
       y = "",
       x = "",
       fill = "Housing Costs")
```
