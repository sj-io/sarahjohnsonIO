...
```{r vUnits}
v_units <- v_acs5 %>% 
  filter(str_starts(name, "B25032_")) %>% 
  mutate(tenure = str_extract(label, "Owner|Renter"), 
         units = str_remove(label, ".*:(!!)?"),
         units = na_if(units, ""),
         unit_sum = case_when(
           is.na(units) ~ NA_character_,
           str_detect(units, "\\b1, ") ~ "Single-family",
           str_detect(units, "\\b[24]\\b") ~ "Duplex-Quadplex",
           str_detect(units, "Mobile|Boat") ~ "Mobile home, boat, etc.",
           TRUE ~ "Larger Multifamily"
         ),
         .after = name) %>% 
  select(-c(label:geography))

units <- hsg %>% 
  filter(str_starts(variable, "B25032_")) %>% 
  left_join(v_units, by = c("variable" = "name")) %>% 
  group_by(GEOID, tenure) %>% 
  mutate(ten_sum = max(estimate)) %>% 
  ungroup()

units_sum <- units %>% 
  filter(!is.na(unit_sum)) %>% 
  mutate(unit_sum = fct_inorder(unit_sum)) %>% 
  group_by(NAME, tenure, unit_sum, ten_sum) %>% 
  summarise(estimate = sum(estimate)) %>% 
  ungroup() %>% 
  mutate(pct = estimate / ten_sum,
         NAME = str_remove_all(NAME, " (city|town).*$"),
         NAME = fct_reorder(NAME, ten_sum, max, .desc = TRUE),
         unit_sum = fct_rev(unit_sum))
```

```{r ggUnits, fig.cap="Units in structure"}
units_sum %>% 
  filter(tenure == "Renter") %>% 
  ggplot(aes(x = unit_sum, y = estimate)) +
  geom_col(fill = "darkcyan") +
  facet_wrap(~NAME, scales = "free_x") +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  coord_flip() +
  theme_minimal() +
  labs(title = "Units in Structure for Renters",
       subtitle = "Places in Shelby County, TN",
       caption = "2017-2021 5-year ACS estimates",
       y = "", x = "")
```
...
```{r}
v_yrblt <- v_acs5 %>% 
  filter(str_starts(name, "B25036")) %>% 
  mutate(tenure = str_extract(label, "Owner|Renter"), 
         yrblt = str_remove(label, ".*:(!!)?"),
         yrblt = na_if(yrblt, ""),
         yrblt = str_remove(yrblt, "Built "),
         yrblt = str_replace(yrblt, " to ", "-"),
         .after = name) %>% 
  select(-c(label:geography))

yrblts <- hsg %>% 
  filter(str_starts(variable, "B25036")) %>% 
  left_join(v_yrblt, by = c("variable" = "name")) %>% 
  group_by(GEOID, tenure) %>% 
  mutate(ten_sum = max(estimate)) %>% 
  ungroup()
...
yrblts_sum <- yrblts %>% 
  filter(!is.na(yrblt)) %>% 
  mutate(yrblt = fct_inorder(yrblt)) %>% 
  group_by(NAME, tenure, yrblt, ten_sum) %>% 
  summarise(estimate = sum(estimate)) %>% 
  ungroup() %>% 
  mutate(pct = estimate / ten_sum,
         NAME = str_remove_all(NAME, " (city|town).*$"),
...
yrblts_sum %>% 
  ggplot(aes(x = tenure, y = estimate, fill = yrblt)) +
  geom_col() +
  facet_wrap(~NAME, scales = "free_x") +
  scale_y_continuous(labels = label_number(scale_cut = cut_short_scale())) +
  coord_flip() +
...
yrblts_sum %>% 
  ggplot(aes(x = tenure, y = pct, fill = yrblt)) +
  geom_col() +
  facet_wrap(~NAME, scales = "free_x") +
